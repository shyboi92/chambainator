...
import {ErrorCodes, Roles, AUTHENTICATED_ROLES, API, API_COURSE, UserInfo, CourseInfo,API_CLASS, HIGHER_ROLES, API_EXERCISE, API_SUBMISSION} from '../inc/constants.js';
...
bindApiWithRoute(API_SUBMISSION.SUBMISSION__CREATE, api => apiRoute(router, api,
	...
	async (req: ApiRequest, res: Response) => {
		...
		if (!req.files?.data_file)
			return req.api.sendError(ErrorCodes.INVALID_PARAMETERS, 'Chưa gửi file dữ liệu');
		
		const originalFileExt = path.extname(req.files.data_file.name).toLowerCase();
		if (!['.c', '.cpp'].includes(originalFileExt))
			return req.api.sendError(ErrorCodes.INVALID_UPLOAD_FILE_TYPE, 'Only C or C++ source file is allowed');

		// de tam o day de test
		fs.writeFileSync(utils.getDataFilePath('new-file.cpp', utils.DataFileTypes.TMP_UPLOAD) as string, req.files.data_file.data);

		const newSubmissionId = (await db.insert('submit', {
		...
		req.ctx.logActivity('New submission', { submission_id: newSubmissionId });
		req.api.sendSuccess({ submission_id: newSubmissionId });
	}
))
