...
app.use(morgan(':remote-addr [:local-date] ":method :url HTTP/:http-version" :status :res[content-length] :response-time ms ":referrer" ":user-agent"', {
	stream: fsRotator.getStream({
		...
		filename: path.join(__dirname, '../../tmp/log/access-%DATE%.log'),
...
if (process.env.NODE_ENV === 'development') console.log(chalk.yellow('Session cookie SameSite is set to None in development mode'));
...
app.use(process.env.WEB_LOCAL_PATH as string, express.static(path.join(__dirname, process.env.NODE_ENV == 'development' ? '../../dist-fe-dev' : '../../dist-fe-prod')));
...
if (process.env.NODE_ENV === 'development') {
	import('../routes/dev-test.js').then(module => {
		app.use(process.env.WEB_LOCAL_PATH as string, module.default as express.Router);
		console.log(chalk.yellow('Test routes are mounted in development mode'));
	});
}
